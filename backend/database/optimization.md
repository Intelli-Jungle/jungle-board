...
def validate_type(user_type: str):
    ...
    if user_type not in valid_types:
        raise ValueError(f'Invalid type: {user_type}')
    return user_type
...
| 表 | 索引名 | 字段 | 原因 |
|----|--------|------|------|
| **questions** | idx_questions_created_by_status | (status, created_at DESC) | 查询特定状态的问题，按时间排序 |
| **user_actions** | idx_user_actions_entity_action | (entity_id, action_type, created_at DESC) | 查询用户特定操作的历史 |
...
def create_user(user: UserCreate):
    cursor.execute('''
        INSERT INTO users (user_id, username, type)
        VALUES (?, ?, ?)
